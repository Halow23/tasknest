...
export function InviteTeammatesButton({ className, variant, size }: { className?: string; variant?: ComponentProps<typeof Button>["variant"]; size?: ComponentProps<typeof Button>["size"] }) {
  ...
  const workspaceInput = useMemo(() => ({ workspaceId: workspaceId ?? "" }), [workspaceId]);
  const pendingInvites = trpc.tasknest.workspace.pendingInvites.useQuery(workspaceInput, { enabled: isAuthenticated && open && Boolean(workspaceId) });
  ...
  const emailInvite = (inviteId: string) => sendInviteEmail.mutate({ inviteId, workspaceId, appOrigin: window.location.origin });
  const openMailApp = (email: string, url: string) => {
    const subject = encodeURIComponent(`You are invited to ${workspace.name} on TaskNest`);
    const body = encodeURIComponent(`Hi,\n\nYou have been invited to collaborate in ${workspace.name} on TaskNest. Use this one-time link to join:\n\n${url}\n\nThis invitation expires in 7 days.\n`);
    ...
    <Dialog open={open} onOpenChange={closeDialog}>
      <DialogContent className="max-h-[88vh] max-w-lg overflow-y-auto">
        ...
        {createInvite.data && <div className="space-y-3 rounded-xl border border-[#DDEAF0] bg-white p-4">
          ...
          <div className="flex flex-wrap gap-2"><Button type="button" size="sm" onClick={copyInvite} variant="outline"><Link2 className="mr-1.5 h-3.5 w-3.5" />Copy link</Button><Button type="button" size="sm" onClick={() => openMailApp(createInvite.data.recipientEmail!, inviteUrl)} className="bg-[#247EAF] hover:bg-[#176A98]"><Mail className="mr-1.5 h-3.5 w-3.5" />Send email</Button><Button type="button" size="sm" variant="outline" disabled={sendInviteEmail.isPending} onClick={() => emailInvite(createInvite.data.id)}><Send className="mr-1.5 h-3.5 w-3.5" />{sendInviteEmail.isPending ? "Sending…" : "Send via Resend"}</Button></div>
        </div>}
        <section className="border-t border-[#E5EDF2] pt-4">
          ...
          <div className="mt-3 space-y-2">{pendingInvites.isLoading ? <div role="status" aria-busy="true" className="rounded-xl border border-dashed border-[#D7E5EB] p-3"><SkeletonRows rows={2} /></div> : (pendingInvites.data as any[])?.length ? (pendingInvites.data as any[]).map(invite => { const existingInviteUrl = `${window.location.origin}/?invite=${invite.token}`; return <article key={invite.id} className="flex items-center gap-3 rounded-xl border border-[#E2EBF0] bg-white p-3"><div className="min-w-0 flex-1"><p className="truncate text-xs font-extrabold text-[#294A62]">{invite.recipientEmail || "Legacy invitation link"}</p><p className="mt-1 text-[10px] font-medium text-[#78909F]">Expires {formatExpiry(invite.expiresAt)}</p></div><div className="flex shrink-0 gap-1"><Hint label="Open email app" side="top"><Button type="button" size="icon" variant="ghost" disabled={!invite.recipientEmail} onClick={() => openMailApp(invite.recipientEmail!, existingInviteUrl)} aria-label={`Open email app for ${invite.recipientEmail || "invitee"}`}><Mail className="h-3.5 w-3.5 text-[#2778A9]" /></Button></Hint><Hint label="Send invite email" side="top"><Button type="button" size="icon" variant="ghost" disabled={!invite.recipientEmail || sendInviteEmail.isPending} onClick={() => emailInvite(invite.id)} aria-label={`Send through Resend to ${invite.recipientEmail || "invitee"}`}><Send className="h-3.5 w-3.5 text-[#2778A9]" /></Button></Hint><Hint label="Revoke invitation" side="top"><Button type="button" size="icon" variant="ghost" disabled={revokeInvite.isPending} onClick={() => revokeInvite.mutate({ inviteId: invite.id, workspaceId })} aria-label={`Revoke invitation for ${invite.recipientEmail || "invitee"}`}><Trash2 className="h-3.5 w-3.5 text-[#D44A3F]" /></Button></Hint></div></article>; }) : <p className="rounded-xl border border-dashed border-[#D7E5EB] bg-[#FBFDFE] p-4 text-center text-xs leading-5 text-[#718A9A]">No active invitations. Create one when a teammate is ready to join.</p>}</div>
        </section>
