...
describe("WorkspaceInviteControl", () => {
  it("exposes invitation creation, direct delivery, mail-app fallback, pending tracking, and revocation controls", async () => {
    ...
    expect(source).toContain("sendInviteEmail.mutate({ inviteId, workspaceId, appOrigin: window.location.origin })");
    ...
    expect(source).toContain("revokeInvite.mutate({ inviteId: invite.id, workspaceId })");
