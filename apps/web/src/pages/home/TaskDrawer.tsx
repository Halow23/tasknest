...
import { TaskDrawerSkeleton } from '@/components/Loading';
...
export function TaskDrawer({ taskId, workspaceId, members, fields, labels, projectTasks, onClose, onDeleted }: { taskId: string | null; workspaceId: string; members: Member[]; fields: ProjectField[]; labels: WorkspaceLabel[]; projectTasks: { id: string; title: string; status: string }[]; onClose: () => void; onDeleted: () => void }) {
  const utils = trpc.useUtils();
  const detail = trpc.tasknest.task.detail.useQuery({ taskId: taskId ?? "", workspaceId }, { enabled: taskId !== null, refetchInterval: 8_000 });
  ...
  const deleteMutation = trpc.tasknest.task.delete.useMutation({ onSuccess: async (result) => { await utils.tasknest.task.list.invalidate(); await utils.tasknest.analytics.project.invalidate(); utils.tasknest.trash.list.invalidate(); toast.success("Task deleted.", { action: { label: "Undo", onClick: () => restoreMutation.mutate({ taskId: result.deletedTaskId, workspaceId }) } }); onDeleted(); }, onError: error => toast.error(error.message) });
  const dependenciesQuery = trpc.tasknest.dependency.list.useQuery({ taskId: taskId ?? "", workspaceId }, { enabled: taskId !== null });
  ...
  const stopTimer = () => {
    ...
    logTimeMutation.mutate({ taskId: taskId ?? "", workspaceId, minutes, note: "Timer session" });
  };
  ...
  const upload = async (file: File | undefined) => {
    ...
    const relayThroughServer = () => {
      ...
      reader.onload = () => uploadMutation.mutate({ taskId, workspaceId, fileName: file.name, contentType, dataBase64: String(reader.result) });
      reader.readAsDataURL(file);
    };
    try {
      const presigned = await presignMutation.mutateAsync({ taskId, workspaceId, fileName: file.name, contentType, byteSize: file.size });
      ...
      await registerMutation.mutateAsync({ taskId, workspaceId, fileName: file.name, contentType, byteSize: file.size, storageKey: uploadResult.public_id, cloudinaryUrl: uploadResult.secure_url });
    } catch {
      toast.info("Direct upload unavailable — sending through the server instead.");
      relayThroughServer();
    }
  };
  return <><Sheet open={taskId !== null} onOpenChange={open => !open && onClose()}><SheetContent className="w-full overflow-y-auto border-l border-[#DDE8EE] bg-[#FBFCFD] p-0 sm:max-w-[540px]">{detail.isLoading && <TaskDrawerSkeleton />}{task && <div><div className="border-b border-[#E4ECF1] bg-white px-6 pb-5 pt-6"><div className="mb-4 flex items-center justify-between gap-3 pr-10"><div className="flex flex-wrap gap-2"><Badge variant="outline" className="border-0 bg-[#EAF6FF] text-[10px] font-bold text-[#2776A5]">{task.project?.name}</Badge><span className={cn("rounded-md px-1.5 py-0.5 text-[10px] font-bold capitalize ring-1 ring-inset", priorityStyle[task.priority])}>{task.priority}</span>{task.recurrenceRule && task.recurrenceRule !== "none" && <Badge variant="outline" className="h-5 gap-1 border-0 bg-[#EEF6FB] px-1.5 text-[10px] font-bold capitalize text-[#31779F]"><Repeat className="h-2.5 w-2.5" />{task.recurrenceRule}</Badge>}</div><span className="font-mono text-[10px] font-bold text-[#91A3AE]">#{task.id}</span></div><SheetHeader className="p-0 text-left"><SheetTitle className="text-[22px] font-extrabold leading-7 tracking-[-0.04em] text-[#172B4D]">{task.title}</SheetTitle><SheetDescription className="sr-only">Task details and team collaboration.</SheetDescription></SheetHeader><div className="mt-5 flex flex-wrap items-center justify-between gap-2"><Faces members={task.assignees} /><div className="flex gap-2"><Hint label="Copy task link" side="left"><Button variant="outline" size="icon" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/?task=${task.id}`).then(() => toast.success("Task link copied to clipboard.")).catch(() => toast.error("Copy failed — copy the URL manually.")); }} className="h-8 w-8 rounded-lg text-[#2778A9] hover:bg-[#EAF6FF]" aria-label="Copy task link"><Link2 className="h-3.5 w-3.5" /></Button></Hint><Button variant="outline" size="sm" onClick={() => setEditOpen(true)} className="h-8 rounded-lg text-[10px]"><Edit3 className="mr-1 h-3.5 w-3.5" />Edit</Button><Button variant="outline" size="sm" onClick={() => moveMutation.mutate({ taskId: task.id, workspaceId, status: task.status === "done" ? "progress" : "done" })} className="h-8 rounded-lg text-[10px]"><Check className="mr-1 h-3.5 w-3.5" />{task.status === "done" ? "Reopen" : "Complete"}</Button><Hint label="Delete task" side="left"><Button variant="outline" size="icon" onClick={() => setDeleteOpen(true)} className="h-8 w-8 rounded-lg text-[#D44A3F] hover:bg-[#FFF0EE]"><Trash2 className="h-3.5 w-3.5" /><span className="sr-only">Delete task</span></Button></Hint></div></div></div><div className="space-y-6 px-6 py-6"><section><p className="section-label">Task brief</p><p className="mt-2 text-[12px] font-medium leading-5 text-[#526B7B]">{task.description || "No task brief has been added yet."}</p></section>{(task.labels ?? []).length > 0 && <section className="flex flex-wrap gap-1.5">{(task.labels ?? []).map(label => <LabelChip key={label.id} label={label} />)}</section>}<section className="grid grid-cols-2 gap-3"><div className="rounded-xl border border-[#E2EBF0] bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#90A1AB]">Due</p><p className="mt-1 text-[12px] font-extrabold text-[#2B526B]">{formatDate(task.dueAt)}</p></div><div className="rounded-xl border border-[#E2EBF0] bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#90A1AB]">Status</p><p className="mt-1 text-[12px] font-extrabold text-[#2B526B]">{columns.find(column => column.id === task.status)?.title}</p></div></section>{task.fieldValues?.length > 0 && <section aria-label="Custom field values">{task.fieldValues.map(item => <div key={item.fieldId} className="rounded-xl border border-[#E2EBF0] bg-white p-3"><p className="text-[9px] font-extrabold uppercase tracking-[0.12em] text-[#90A1AB]">{item.name}</p><p className="mt-1 truncate text-[12px] font-extrabold text-[#2B526B]">{item.type === "date" && item.value ? formatDate(new Date(`${item.value}T00:00:00`)) : item.value}</p></div>)}</section>}<section aria-label="Blocked by">
...
{(task.openDependencies ?? []).map((dependency: any) => <div key={dependency.id} className="flex items-center gap-2 rounded-xl border border-[#E2EBF0] bg-white p-3"><Lock className="h-3.5 w-3.5 shrink-0 text-[#A36A00]" /><span className="min-w-0 flex-1 truncate text-[11px] font-extrabold text-[#395269]">{dependency.title}</span><button type="button" disabled={removeDependency.isPending} onClick={() => removeDependency.mutate({ taskId: task.id, workspaceId, dependsOnTaskId: dependency.id })} className="shrink-0 rounded-md p-1 text-[#7B8F9C] hover:bg-[#FFF0EE] hover:text-[#D44A3F]" aria-label={`Remove dependency ${dependency.title}`}><X className="h-3 w-3" /></button></div>)}
</div>
...
<Button size="sm" disabled={!dependencySelect || addDependency.isPending} onClick={() => addDependency.mutate({ taskId: task.id, workspaceId, dependsOnTaskId: dependencySelect })} className="h-8 shrink-0 bg-[#38A9F2] text-[10px] hover:bg-[#248FCC]">Link</Button>
</div>
</section>
<section><div className="flex items-center justify-between"><p className="section-label">Subtasks</p><span className="text-[10px] font-bold text-[#78909F]">{task.subtasks.filter(item => item.completed).length}/{task.subtasks.length} done</span></div><div className="mt-2 overflow-hidden rounded-xl border border-[#E2EBF0] bg-white">{task.subtasks.length === 0 && <p className="p-3 text-[11px] text-[#8B9EAA]">Break this work into a few clear next moves.</p>}{task.subtasks.map(item => <label key={item.id} className="flex cursor-pointer items-center gap-2.5 border-b border-[#EEF3F5] px-3 py-3 last:border-0"><Checkbox checked={item.completed} onCheckedChange={checked => toggleMutation.mutate({ taskId: task.id, workspaceId, subtaskId: item.id, completed: checked === true })} /><span className={cn("text-[11px] font-semibold", item.completed ? "text-[#8C9EA9] line-through" : "text-[#395269]")}>{item.title}</span></label>)}</div><div className="mt-2 flex gap-2"><Input value={subtask} onChange={event => setSubtask(event.target.value)} onKeyDown={event => { if (event.key === "Enter" && subtask.trim()) subtaskMutation.mutate({ taskId: task.id, workspaceId, title: subtask }); }} placeholder="Add a next move…" className="h-8 text-[11px]" /><Button size="sm" disabled={!subtask.trim() || subtaskMutation.isPending} onClick={() => subtaskMutation.mutate({ taskId: task.id, workspaceId, title: subtask })} className="h-8 bg-[#38A9F2] text-[10px] hover:bg-[#248FCC]">Add</Button></div></section><section aria-label="Time tracking">
