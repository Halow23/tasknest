    ...
    <Dialog open={open} onOpenChange={closeDialog}>
      <DialogContent className="max-h-[88vh] max-w-lg overflow-y-auto">
        ...
        {createInvite.data && <div className="space-y-3 rounded-xl border border-[#DDEAF0] bg-white p-4">
          ...
          <div className="flex flex-wrap gap-2"><Button type="button" size="sm" onClick={copyInvite} variant="outline"><Link2 className="mr-1.5 h-3.5 w-3.5" />Copy link</Button><Button type="button" size="sm" onClick={() => openMailApp(createInvite.data.recipientEmail, inviteUrl)} className="bg-[#247EAF] hover:bg-[#176A98]"><Mail className="mr-1.5 h-3.5 w-3.5" />Open email app</Button><Button type="button" size="sm" variant="outline" disabled={sendInviteEmail.isPending} onClick={() => emailInvite(createInvite.data.id)}><Send className="mr-1.5 h-3.5 w-3.5" />{sendInviteEmail.isPending ? "Sending…" : "Send via Resend"}</Button></div>
        </div>}
