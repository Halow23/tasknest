...
describe("real-time workspace events", () => {
  it("publishes every activity event to the in-process emitter", async () => {
    const emitter = await readFile(new URL("../../../server/events.ts", import.meta.url), "utf8");
    const router = await readFile(new URL("../../../server/routers/tasknest.ts", import.meta.url), "utf8");
  ...
  it("streams authenticated workspace-scoped SSE events with heartbeats", async () => {
    const route = await readFile(new URL("../../../server/_core/workspaceEvents.ts", import.meta.url), "utf8");
